...
class UserController:
    ...
    async def register_user(
        ...
        try:
            exising_user = await db.scalar(select(UserModel).where(
            ...
            await db.rollback()
            logger.exception(
                "Unexcepted error while registering user."
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def update_profile(
        ...
        try:
            ...
            logger.info(
            "Profile updated successfully | user_id=%s",
            current_user.id)
    ...
    async def change_password(
        ...
        try:
            logger.info(
            "Password change requested | user_id=%s",
            current_user.id)
